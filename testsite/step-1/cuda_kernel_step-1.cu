...
// @param leading_dim : length of a row/column in memory.
template<typename T>
__global__
void
                       ...
                       int n_cols, int leading_dim)
{
    ...
    for(int k=0; k < k_max; k++)
    {
        ...
        // Now compute the  diagonal element and the row to its right-hand side
        // \f{eqnarray}
...
__strip_update(T *A, int n_blocks_done, int n_cols, int leading_dim)
{
    ...
    // The "+1" is needed since @p n_blocks_done denotes the position of the
    // last preceding factorized diagonal block w.r.t. the
    // left uppermost block.
    ...
    // Get the transposed block which is to be processed
...
//
// The factorization is in-place. At the end the Cholesky factor is
// contained in the lower triangle (including the diagonal). The off-diagonals
// above the diagonal still contain the entries of the original matrix.
//
// @param a_d : Pointer to linear array in device memory containing the matrix elements.
// @param n_cols : number of rows/columns of the matrix.
// @param leading_dim : length of a row/column in memory.
template<typename T>
...
//
// For the parameters see the corresponding kernel.
template<typename T>
