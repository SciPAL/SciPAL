...
// of this class.
// Because of one of its attributes we have to add the spatial dimension as template parameter.
template<int dim>
class Timing {

public:

    Timing( step27::NumericsParams<dim> &numerics_params, const uint dofs_per_be,
            const uint n_cells,
            const uint n_bem_points, const uint cycle);
    ...
    // @p Step16::DRSProblem in drs_simulation.h
...
template<int dim>
step28::Timing<dim>::Timing(step27::NumericsParams<dim> &numerics_params,
...
// are written.
template<int dim>
void step28::Timing<dim>::add_values_to_table(dealii::ConvergenceTable &table)
{



    this->average_iteration_time /= this->iteration_counter;
    qDebug() << "Average: " << this->average_iteration_time << "s"
             << "\tTotal: " << this->total_time << "s";


    table.add_value("#cycle", cycle);
    table.add_value("n_threads", numerics_params.n_threads);
    table.add_value("n_dofs_per_be", dofs_per_be);
    table.add_value("n_cells", n_cells);
    table.add_value("n_bem_quad_points", (int)numerics_params.bem_q_rule_2->get_points().size());
    table.add_value("n_bem_inner_quad_points",  (int)numerics_params.bem_q_rule_1->get_points().size());
    ...
    table.add_value("avg_iter_time", average_iteration_time);
