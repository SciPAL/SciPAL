...
#ifndef CUDA_DRIVER_STEP_TEMPLATE_HH
#define CUDA_DRIVER_STEP_TEMPLATE_HH


// The declaration of the interface to the CUDA-backend
// is contained in the following header.
#include <cuda_kernel_wrapper_step-template.cu.h>
...
steptemplate::CUDADriver::CUDADriver(float * v_h, int n)
    :
    __n(n),
...
steptemplate::CUDADriver::~CUDADriver()
{
   cudaFree(__values_d);
    __values_h = 0;
}
...
void steptemplate::CUDADriver::run()
{
    ...
    steptemplate::Kernels<float> backend;
...
#endif // CUDA_DRIVER_STEP_TEMPLATE_HH
