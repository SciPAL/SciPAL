...
#include <step-template/cuda_kernel_wrapper_step-template.cu.h>
...
template<typename T>
__global__ void __step_template_dummy()
{
    int i = 2;
    int j = 3;
    int k = i+j;
...
// @sect4{Function: step_template_dummy}
//
...
template<typename T>
void steptemplate::Kernels<T>::dummy()
{
    dim3 grid(2,1);
    dim3 blocks(16,1);
    ...
    __step_template_dummy<T><<<grid, blocks>>>();
...
template class steptemplate::Kernels<float>;
template class steptemplate::Kernels<double>;
